...
std::unordered_map<string, string> user_colors;
int color_index = 0;
...
int chat_recv() {
    ...
    while (isServerConnected) {
        ...
        if (recv(client_sock, buf, MAX_SIZE, 0) > 0) {
            ...
            string user;
            ss >> user;
            if (user_colors.find(user) == user_colors.end()) {
                user_colors[user] = "\033[3" + std::to_string(color_index) + "m";
                color_index = (color_index + 1) % 10;
            }

            if (user != my_nick)
                cout << user_colors[user] << buf << "\033[0m" << endl;
        }
        else {
